...
async function query(collectionName) {
    ...
    var collection = await storageService.load(collectionName)

    if (!collection || !collection.length) {
        collection = _createDefaultContacts()
        await storageService.save(KEY, collection)
    }
    collection = await getContacts(collection, user)
...
function getContacts(data, { contacts }) {
    if (!contacts) return []
    return data.filter(contact => contacts.indexOf(contact._id) !== -1)
}
